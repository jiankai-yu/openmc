...
from openmc.data import HDF5_VERSION, HDF5_VERSION_MAJOR
from openmc.data.ace import Library, Table, get_table, get_metadata
from openmc.data.data import ATOMIC_SYMBOL, K_BOLTZMANN, EV_PER_MEV
from openmc.data.endf import (
    Evaluation, SUM_RULES, get_head_record, get_tab1_record, get_evaluations)
from openmc.data.fission_energy import FissionEnergyRelease
from openmc.data.function import Tabulated1D, Sum, ResonancesWithBackground
from openmc.data.grid import linearize, thin
from openmc.data.njoy import make_ace
from openmc.data.product import Product
from openmc.data.reaction import Reaction, _get_photon_products_ace, FISSION_MTS
from openmc.data import resonance as res
from openmc.data import resonance_covariance as res_cov
...
class IncidentHelium3(EqualityMixin):
    ...
    :meth:`IncidentHelium3.from_hdf5`, :meth:`IncidentHelium3.from_ace`, and
    :meth:`IncidentHelium3.from_endf`.
    ...
    def __repr__(self):
        return "<IncidentHelium3: {}>".format(self.name)
    ...
    def add_temperature_from_ace(self, ace_or_filename, metastable_scheme='nndc'):
        ...
        data = IncidentHelium3.from_ace(ace_or_filename, metastable_scheme)
    ...
    def from_ace(cls, ace_or_filename, metastable_scheme='nndc'):
        ...
        if not xs.endswith('s'):
            raise TypeError(
                "{} is not a continuous-energy helium3 ACE table.".format(ace))
    ...
    def from_njoy(cls, filename, temperatures=None, evaluation=None, **kwargs):
        ...
        **kwargs
        ...
        Returns
        -------
        data : openmc.data.IncidentHelium3
            Incident neutron continuous-energy data
    ...
    def add_stopping_power(self, csdb):
        strT = self.temperatures[0]
        energy = self.energy[strT]
        stopping = Reaction(500)
        stopping.xs[strT] = Tabulated1D(energy, csdb)
        stopping.redundant = False
        self.reactions[500] = stopping
        
    def add_range(self, ranges):
        strT = self.temperatures[0]
        energy = self.energy[strT]
        rangexs = Reaction(999)
        rangexs.xs[strT] = Tabulated1D(energy, ranges)
        rangexs.redundant = False
        self.reactions[999] = rangexs